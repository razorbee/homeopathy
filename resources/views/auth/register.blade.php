    ...
    <div class="card-box form-zoom-in-up {{ $errors->has('email') || $errors->has('password') || $errors->has('name') ? 'form-shake' : '' }}">
        ...
        <div class="p-20">
            <form class="form-horizontal m-t-20" action="{{ route('register') }}" method="POST">
                {{csrf_field()}}
                <div class="form-group-custom{{$errors->has('name') ? 'has-error':''}}">
				...
				<div class="form-group-custom{{$errors->has('username') ? 'has-error':''}}">
                    <input type="type" id="user-name" name="username" value="{{old('username')}}" required="required"/>
